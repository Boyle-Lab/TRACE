...
```
To make sure that correct version of Python is used and all required packages are installed, we recommend using Conda and creating the environment from the `environment.yml` file: 
...
- [`<peak_7.file>`](data/E2F1_peak_7.bed): A file containing regions of interest and motifs sites inside these regions. The first 3 columns and next 3 columns are chromosome number, start position and end position of region of interest and motif site inside that region, the last column is number of bases overlapping between motif site and peak, which can be easily obtained by BEDTools intersect.   
